...
3. **Check node list first** — call `ssh_list_nodes` before executing if unsure which nodes exist
4. **Respect confirm prompts** — always show confirm messages to the user, never auto-bypass
5. **Set reasonable timeouts** — default is 30s, increase for long-running commands (`timeout=300`)
6. **Use tags** — nodes can have tags like `["gpu", "prod"]` for organization
